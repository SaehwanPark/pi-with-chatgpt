...
| INV-09 | Task conversations are isolated and writes are serialised | `chatgpt/scope.ts` (`conversationKeyForTask`) + `chatgpt/conversation-mapping.ts` (`KeyedMutex`) + `chatgpt/conversation-recovery.ts` (cross-process lock and same-Project replacement) + `browser/playwright-driver.ts` (driver-owned lock across the shared tab) + `browser/transaction.ts` (process-wide transaction covering capability probes through receipt) |
| INV-10 | No silent OpenAI/ChatGPT account switch | `auth/identity.ts` (**single authority** `resolveAccountIdentity` answers every case; `compareAccountIdentity`, namespace-scoped `accountIdNamespace`, pair-bound `decideAccountMismatch` — only a `keep-current` minted for this account pair resolves a mismatch, and there is no boolean override), `auth/adviser-auth.ts` (`resolveAdviserAuth` delegates to it and holds no second copy of the rule), `auth/readiness.ts` (production command/tool dispatch feeds fresh browser observations through the resolver, refuses demonstrated mismatches, and refuses a signed-in browser with no comparable identity) |
