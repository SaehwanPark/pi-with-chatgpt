...
| INV-09 | Task conversations are isolated and writes are serialised | `chatgpt/scope.ts` (`conversationKeyForTask`) + `chatgpt/conversation-mapping.ts` (`KeyedMutex`) + `chatgpt/conversation-recovery.ts` (cross-process lock and same-Project replacement) |
...
only the runtime's already-owned tab and returns a structured refusal when the ChatGPT surface cannot be
recognised. Live ChatGPT interaction remains a manual validation drill, not a unit-test dependency.
